    ...
    def run(self, timeout):
        def target():
            ...
            self.process = subprocess.Popen(
                "/usr/bin/php " + program_path + "/../../core/php/newpush.php " + self.api_key, shell=True)
...
def shutdown():
    ...
    logger.debug(f'Removing PID file {pidfile}')
    os.remove(pidfile)
...
def handler(signum=None, frame=None):
    logger.debug(f'Signal {signum} caught, exiting...')
    shutdown()
...
def daemonize():
    try:
        ...
        raise RuntimeError(f'1st fork failed: {e.strerror} [{e.serrno}]')
...
def logger_init(name, debug):
    program_path = os.path.dirname(os.path.realpath(__file__))
    logfile = program_path + '/../../../../log/pushbullet_daemon.log'
    loglevel = 'ERROR'
...
def main():
    ...
    logger.debug(f'PID file {pidfile}')

    if os.path.exists(pidfile):
        print(f'PID file \'{pidfile}\' already exists. Exiting.')
        logger.debug(f'PID file \'{pidfile}\' already exists.')
        logger.debug("Exit 1")
        sys.exit(1)

    try:
        logger.debug(f'Write PID file: {pidfile}')
        with open(pidfile, 'w') as f:
            f.write(f'{os.getpid()}\n')
